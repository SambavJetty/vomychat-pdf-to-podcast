...
class AudioGenerator:
    def __init__(self):
        self.VOICE_ID = "OJtLHqR5g0hxcgc27j7C"
        self.OUTPUT_DIR = Path.home() / "assistant" / "audio"
    ...
    def check_dependencies(self, debug=False):
    ...
    def get_text_content(self, args):
        text_content = ""
        if args.clipboard:
            if not pyperclip:
    ...
    def generate_speech(self, text_content, debug=False):
        headers = {
            "Accept": "audio/mpeg",
    ...
    def save_audio(self, content):
        timestamp = int(time.time())
        output_file = self.OUTPUT_DIR / f"{timestamp}_speech.mp3"
        try:
    ...
    def main(self):
        ...
        self.check_dependencies(debug=args.debug)
        self.create_output_dir()

        text_content = self.get_text_content(args)

        if args.debug:
            print("Debug: Input text content:")
            print(text_content)

        audio_content = self.generate_speech(text_content, debug=args.debug)
        if not audio_content:
            print("Error: Speech generation failed")
            sys.exit(1)

        output_file = self.save_audio(audio_content)
        if not output_file:
